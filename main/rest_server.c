...
void enable_cors(httpd_req_t *r) {
    httpd_resp_set_hdr(r, "Access-Control-Allow-Origin", "http://192.168.0.102");
}
...
// /* Get devices controller */
// static esp_err_t ipconfig_get_handler(httpd_req_t *req) {
//     httpd_resp_set_type(req, "application/json");
//     get_devices_ip_adresses();
//     cJSON * root = cJSON_CreateObject();
//     cJSON_AddStringToObject(root, "ip", IP2STR(ip_config.ip));
//     char * sys_info = cJSON_PrintUnformatted(root);
//     httpd_resp_sendstr(req, sys_info);
//     return ESP_OK;
// }

/* Delete devices controller */
static esp_err_t devices_delete_handler(httpd_req_t *req) {
    // enable_cors(req);
    httpd_resp_set_type(req, "application/json");
    const char * url = req->uri;
    int device_id = atoi(strrchr(url, '/')+1);
    if (delete_from_array("devices", device_id)) {
        httpd_resp_sendstr(req, "{\"isError\": false}");
        return ESP_OK;
    }
    httpd_resp_sendstr(req, "{\"isError\": true}");
    return ESP_FAIL;
}

/* Edit devices controller */
static esp_err_t devices_put_handler(httpd_req_t *req) {
    ...
    const char * url = req->uri;
    int device_id = atoi(strrchr(url, '/')+1);
    char * edited_device = cJSON_PrintUnformatted(edit_object_in_array("devices", device_id, buf));
    httpd_resp_sendstr(req, edited_device);
    return ESP_OK;
}
...
static esp_err_t devices_post_handler(httpd_req_t *req)
{
    ...
    buf[total_len] = '\0';
    ESP_LOGI("DATA", "%s", buf);
    if (add_to_array_nvs("devices", buf)) {
        httpd_resp_sendstr(req, "Post control value successfully");
        return ESP_OK;
    } else {
        httpd_resp_sendstr(req, "Post failed!");
        return ESP_FAIL;
    }
}
...
/**
    ...
    // /* Get ip configs */
    // httpd_uri_t ip_get_uri = {
    //     .uri = "/api/v1/ip",
    //     .method = HTTP_GET,
    //     .handler = ipconfig_get_handler,
    //     .user_ctx = rest_context
    // };
    // httpd_register_uri_handler(server, &ip_get_uri);
